...
import com.axonivy.utils.axonivyexpress.service.AdhocHistoryService;
...
public class AdhocUtils {
  ...
  public static String getDescriptionOfFirstTask(
      ExecutePredefinedWorkflowData data) {
    Object dynaData = Optional.ofNullable(data.getCurrentTask())
        .map(TaskDef::getDynaFormController).map(DynaFormController::getModel)
        .map(DynaFormModel::getControls).map(controls -> controls.get(0))
        .map(DynaFormControl::getData).orElse(null);
    return Optional.ofNullable((Formelement) dynaData)
        .map(Formelement::getValue).map(String::valueOf).orElse(null);
  }
  ...
  public static void storeHistory(long originalTaskId, String content) {
    ...
    adhocHistory
        .setExternalSecurityId(Ivy.wfTask().getWorkerUser().getExternalId());
    AdhocHistoryService adhocHistoryService = new AdhocHistoryService();
    adhocHistoryService.save(adhocHistory);
  }

  public static void attachToBusinessCase(final ICase icase,
      final Long businessCaseId) {
    Sudo.get(() -> {
