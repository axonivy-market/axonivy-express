...
import com.axonivy.utils.axonivyexpress.entity.FormelementOption;
import com.axonivy.utils.axonivyexpress.enums.FormElementType;
import com.axonivy.utils.axonivyexpress.enums.UploadFileFormat;
...
public class DynaFormController implements Serializable {
  ...
  public void createForm() {
    ...
    initializeHeaderOrFooterSection(
        dragAndDropController.getSelectedFormelementsHeader());
    initializeMainSections();
    initializeHeaderOrFooterSection(
        dragAndDropController.getSelectedFormelementsFooter());
  }

  /**
   * Loop to create all elements places in the Header or Footer Section
   */
  private void initializeHeaderOrFooterSection(
  ...
  private void initializeMainSections() {
    ...
    for (int i = 0; i < getMax(
        dragAndDropController.getSelectedFormelementsLeftPanel().size(),
        dragAndDropController.getSelectedFormelementsRightPanel()
            .size()); i++) {
      ...
      if (dragAndDropController.getSelectedFormelementsLeftPanel().size() > i) {
        Formelement element = dragAndDropController
            .getSelectedFormelementsLeftPanel().get(i);
        dragAndDropController.addFinalUsedFormelements(element);

        DynaFormLabel label = labelRow.addLabel(element.getDisplayLabel(), 2,
            1);

        DynaFormControl control = controlRow.addControl(element,
            element.getType().getValue(), 2, 1);
        label.setForControl(control);
      } else {
      ...
      if (dragAndDropController.getSelectedFormelementsRightPanel()
          .size() > i) {
        Formelement element = dragAndDropController
            .getSelectedFormelementsRightPanel().get(i);
        dragAndDropController.addFinalUsedFormelements(element);
  ...
  public String submitForm() {
    ...
    PrimeRequestContext.getCurrentInstance().getCallbackParams().put("isValid",
        !hasErrors);
  ...
  public void initializeDefaultUI() {
    ...
    description.setLabel(
        Ivy.cms()
            .co("/Dialogs/com/axonivy/portal/components/Common/Description"));
