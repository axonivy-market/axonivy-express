...
public class ApprovalResultBean implements Serializable {
  ...
  public void groupingApprovalResult(List<ApprovalTaskResult> approvalResults) {
    ...
    Map<Integer, List<ApprovalTaskResult>> approvalResultMap = approvalResults
        .stream().collect(Collectors.groupingBy(ApprovalTaskResult::getIndex));
    for (Map.Entry<Integer, List<ApprovalTaskResult>> entry : approvalResultMap
        .entrySet()) {
  ...
  public void setApprovalResultGroups(
      List<List<ApprovalTaskResult>> approvalResultGroups) {
