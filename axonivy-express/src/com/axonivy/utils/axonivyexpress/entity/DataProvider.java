package com.axonivy.utils.axonivyexpress.entity;
...
public class DataProvider {
  ...
  public List<String> execute() {
    ...
    Ivy.log()
        .debug("call data provider " + subProcessStart.getProjectName() + "/"
            + subProcessStart.getProcessPath() + "/"
  ...
  private ISubProcessStart findSubProcessStart(List<String> optionStrs) {
    ...
    SubProcessSearchFilter configuration = SubProcessSearchFilter.create()
        .setLibraryId(optionStrs.get(0)).setSignature(optionStrs.get(1))
        .toFilter();

    List<ISubProcessStart> subProcessStarts = SubProcessRunner
        .findSubProcessStarts(configuration);
    return subProcessStarts.stream().filter(this::hasCorrectResultParameter)
        .findAny().orElse(null);
  }
