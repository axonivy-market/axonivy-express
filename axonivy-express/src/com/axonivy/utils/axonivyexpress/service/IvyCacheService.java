...
public class IvyCacheService {
  ...
  /**
   * Get value of a session cache based on groupIdentifier and identifier Will
   * return empty if this key doesn't exist in session or invalid any more
   * 
   * @param groupIdentifier
   * @param identifier
   * @return value of cache
   */
  public Optional<Object> getSessionCacheValue(String groupIdentifier,
      String identifier) {
    verifyIdentifier(groupIdentifier, identifier);

    IDataCacheEntry cacheEntry = sessionCache().getEntry(groupIdentifier,
        identifier);
  ...
  /**
   * Invalidate the data of entry in session cache entry with key is
   * groupIdentifier and identifier
   * 
   * @param groupIdentifier
   * @param identifier
   */
  public void invalidateSessionEntry(String groupIdentifier,
      String identifier) {
    verifyIdentifier(groupIdentifier, identifier);
    IDataCacheGroup group = sessionCache().getGroup(groupIdentifier);
    if (group != null) {
      IDataCacheEntry entry = sessionCache().getEntry(groupIdentifier,
          identifier);
      if (entry != null) {
  ...
  /**
   * Add or update an object to session cache with key is groupIdentifier and
   * identifier
   * 
   * @param groupIdentifier
   * @param identifier
   * @param value
   */
  public void setSessionCache(String groupIdentifier, String identifier,
      Object value) {
    verifyIdentifier(groupIdentifier, identifier);
