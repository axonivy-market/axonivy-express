...
import com.axonivy.utils.axonivyexpress.utils.Attrs;
import com.axonivy.utils.axonivyexpress.utils.CaseUtils;
...
public class ExpressBusinessViewBean implements Serializable {
  ...
  @PostConstruct
  public void init() {
    String uuid = Attrs.currentContext().getAttribute("#{data.uuid}",
        String.class);
    this.caseId = Optional.ofNullable(CaseUtils.findCase(uuid))
        .map(ICase::getId).orElse(null);
    expressCase = ExecutingExpressProcessUtils.getExpressCase(caseId);
    if (expressCase != null) {
      executeWorkflowDataList = collectPredefinedWorkflowData(
  ...
  private List<ExecutePredefinedWorkflowData> collectPredefinedWorkflowData(
      String caseCategory) {
    List<Object> taskEndProcessList = ExecutingExpressProcessUtils
        .getExpressTaskEndProcessData(caseId, caseCategory);
  ...
  private void updateActualStepIndex(
      List<ExecutePredefinedWorkflowData> executeWorkflowDataList,
      ICase expressCase) {
    if (CollectionUtils.isNotEmpty(executeWorkflowDataList)) {
      ...
      finishedActualStepIndex = CollectionUtils
          .emptyIfNull(executeWorkflowDataList).stream()
          .map(ExecutePredefinedWorkflowData::getActualStepIndex)
          .mapToInt(index -> index).max().orElseThrow();
    }
  }
  ...
  public void initializeTaskDefData(
      List<ExecutePredefinedWorkflowData> executeWorkflowDataList) {
    finishedTasks = new ArrayList<>();
    ...
    for (ExecutePredefinedWorkflowData executeDefinedWorkflowData : executeWorkflowDataList) {
      ...
      if (taskDef.getTaskType() == TaskType.APPROVAL) {
        approvalTaskResults
            .add(executeDefinedWorkflowData.getApprovalTaskResult());
      } else {
        // Find responsible
        if (taskDef.getActualApplicantName() != null) {
          taskDef.setActualApplicant(
              findActualApplicantByUsername(taskDef.getActualApplicantName()));
  ...
  private IUser findActualApplicantByUsername(String userName) {
    return UserUtils.findUser(userName);
  }
