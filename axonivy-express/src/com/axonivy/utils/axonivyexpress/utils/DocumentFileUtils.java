...
import com.axonivy.utils.axonivyexpress.transformer.IvyDocumentTransformer;
...
public class DocumentFileUtils {
  ...
  private static final String EXPRESS_UPLOAD_FOLDER = "AxonIvyExpress";
  ...
  public static boolean doesDocumentExist(ICase currentCase, String filename) {
    ...
    IDocument document = documentService.get(
        new Path(EXPRESS_UPLOAD_FOLDER).append(filename));
  ...
  public static List<IvyDocument> expressDocuments(ICase currentCase) {
    ...
    IDocumentService documentService = currentCase.documents();
    Path path = new Path(EXPRESS_UPLOAD_FOLDER);
  ...
  public static boolean uploadExpressDocument(ICase currentCase,
      InputStream inputStream, String fileName) {
    IDocumentService documentService = currentCase.documents();
    String originalFileName = FilenameUtils.getName(fileName);
    try {
      documentService.add(
          EXPRESS_UPLOAD_FOLDER + "/" + originalFileName)
          .write().withContentFrom(inputStream);
      return true;
    } catch (Exception e) {
      Ivy.log().error(e);
      return false;
