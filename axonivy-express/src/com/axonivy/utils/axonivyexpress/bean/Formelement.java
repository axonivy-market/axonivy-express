...
import com.axonivy.utils.axonivyexpress.entity.DataProvider;
...
public class Formelement implements Serializable {
  ...
  private List<FormelementOption> options; // list of options for ManyCheckbox
                                           // or OneMenu, but also allowed
                                           // File-Formats
  private FormElementType type; // Form element type
  private List<String> optionsStr; // String List representation of options
  private int taskPosition; // Position of the task which this element belonged
                            // to
  private int indexInPanel; // Index of element in panel (Header, Footer, Left
                            // or Right)
  ...
  public List<String> getOptionsStr() {
    ...
    if (type == FormElementType.CHECKBOX && options != null
      ...
      optionsStr = executeDataProvider(options.stream()
          .map(FormelementOption::getValue).collect(Collectors.toList()));
    } else {
