...
public class IvyDocumentTransformer {
  public IvyDocument transform(IDocument document) {
    try {
      ...
      return IvyDocument.builder().id(String.valueOf(document.getId()))
          .name(document.getName()).path(document.getPath().asString())
          .size(document.getSize()).creation(document.getCreation())
