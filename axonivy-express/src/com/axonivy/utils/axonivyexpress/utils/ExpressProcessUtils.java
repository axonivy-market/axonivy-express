...
import com.axonivy.utils.axonivyexpress.bean.DragAndDropController;
import com.axonivy.utils.axonivyexpress.bean.Formelement;
import com.axonivy.utils.axonivyexpress.controller.DynaFormController;
import com.axonivy.utils.axonivyexpress.entity.ExpressAttachment;
...
import com.axonivy.utils.axonivyexpress.enums.ExpressEmailAttachmentStatus;
import com.axonivy.utils.axonivyexpress.enums.FormElementType;
import com.axonivy.utils.axonivyexpress.enums.TaskType;
import com.axonivy.utils.axonivyexpress.mail.MailAttachment;
...
public class ExpressProcessUtils {
  ...
  public static ExpressProcess saveOrUpdateWorkflow(Data expressData) {
    ExpressProcess processSaved = ExpressProcessService.getInstance()
        .findById(expressData.getProcessID());
    ...
    if (StringUtils.isBlank(processSaved.getProcessOwner())) {
      ...
      processSaved.setProcessOwner(ExpressManagementUtils
          .updateExternalIdToSecurityMemberName(ownerName));
    }

    processSaved.setProcessPermissions(
        ExpressManagementUtils.updateExternalIdsToSecurityMemberNames(
            expressData.getDefinedTasks().get(0).getResponsibles()));
    processSaved.setProcessCoOwners(
        ExpressManagementUtils.updateExternalIdsToSecurityMemberNames(
            expressData.getProcessCoOwners()));
    processSaved.setProcessFolder(expressData.getProcessFolder());
    processSaved.setReadyToExecute(expressData.getReadyToExecute());
    processSaved
        .setTaskDefinitions(buildDefinedTasks(expressData.getDefinedTasks()));
   ...
   */
  private static List<ExpressTaskDefinition> buildDefinedTasks(
      List<TaskDef> definedTasks) {
    ...
    for (TaskDef taskDef : definedTasks) {
      ...
      taskDefinition.setResponsibles(ExpressManagementUtils
          .updateExternalIdsToSecurityMemberNames(taskDef.getResponsibles()));
      ...
      if (taskDef.getTaskType() != TaskType.EMAIL
          && taskDef.getTaskType() != TaskType.APPROVAL) {
        taskDefinition.setFormElements(
            buildExpressFormElements(taskDef.getDragAndDropController()));
      }
   ...
   * @param controller
   */
  private static List<ExpressFormElement> buildExpressFormElements(
      DragAndDropController controller) {
    List<ExpressFormElement> expressFormElements = new ArrayList<>();
    expressFormElements.addAll(createElementsByLocation(
        controller.getSelectedFormelementsHeader(), HEADER_PANEL));
    expressFormElements.addAll(createElementsByLocation(
        controller.getSelectedFormelementsLeftPanel(), LEFT_PANEL));
    expressFormElements.addAll(createElementsByLocation(
        controller.getSelectedFormelementsRightPanel(), RIGHT_PANEL));
    expressFormElements.addAll(createElementsByLocation(
        controller.getSelectedFormelementsFooter(), FOOTER_PANEL));
    return expressFormElements;
  }

  private static List<ExpressFormElement> createElementsByLocation(
      List<Formelement> formElements, String location) {
    List<ExpressFormElement> expressFormElements = new ArrayList<>();
    if (CollectionUtils.isNotEmpty(formElements)) {
      for (Formelement element : formElements) {
        ...
        expressFormElement.setIntSetting(
            Optional.ofNullable(element.getIntSetting()).orElse(0));
        expressFormElement.setLabel(element.getLabel());
        expressFormElement.setRequired(
            Optional.ofNullable(element.getRequired()).orElse(false));
  ...
  public static List<TaskDef> convertExpressTaskDefinitionToTaskDef(
    ...
    for (ExpressTaskDefinition expressTaskDef : expressTaskDefinitions) {
      ...
      taskDef.setResponsibles(
          getValidSecurityMembers(expressTaskDef.getResponsibles()));
      taskDef.setTaskType(
          EnumUtils.getEnum(TaskType.class, expressTaskDef.getType()));
      ...
      taskDef.setResponsibleDisplayName(
          generateResponsibleDisplayName(taskDef.getResponsibles()));
   ...
   */
  public static String generateResponsibleDisplayName(
      List<String> responsibleNames) {
    return CollectionUtils.emptyIfNull(responsibleNames).stream()
        .map(responsibleName -> ISecurityContext.current().members()
            .find(responsibleName))
        .filter(securityMember -> securityMember != null)
        .map(securityMember -> SecurityMemberDisplayNameUtils
            .generateBriefDisplayNameForSecurityMember(securityMember,
   ...
   */
  private static List<String> getValidSecurityMembers(
      List<String> responsibleNames) {
    ...
    responsibleNames.forEach(responsibleName -> {
      String validMemberName = ExpressManagementUtils
          .getValidMemberName(responsibleName);
  ...
  public List<String> getRecipientEmailAddresses(
      List<String> responsibleNames) {
    return CollectionUtils.emptyIfNull(responsibleNames).stream().map(
        responsibleName -> getEmailAddressFromSecurityMember(responsibleName))
   ...
   */
  private static TaskDef initializeControllersForTaskDef(TaskDef taskDef,
      ExpressTaskDefinition expressTaskDefinition) {
    DragAndDropController dragAndDropController = new DragAndDropController();
    DynaFormController dynaFormController = new DynaFormController(
  ...
  private static void updateDragAndDropController(TaskDef taskDef,
      ExpressTaskDefinition expressTaskDefinition) {
    DragAndDropController controller = taskDef.getDragAndDropController();
    List<ExpressFormElement> expressFormElements = expressTaskDefinition
        .getFormElements();
  ...
  private static void sortIndexInPanels(DragAndDropController controller) {
    controller.setSelectedFormelementsHeader(
        controller.getSelectedFormelementsHeader().stream()
            .sorted(Comparator.comparingInt(Formelement::getIndexInPanel))
            .collect(Collectors.toList()));
    controller.setSelectedFormelementsLeftPanel(
        controller.getSelectedFormelementsLeftPanel().stream()
            .sorted(Comparator.comparingInt(Formelement::getIndexInPanel))
            .collect(Collectors.toList()));
    controller.setSelectedFormelementsRightPanel(
        controller.getSelectedFormelementsRightPanel().stream()
            .sorted(Comparator.comparingInt(Formelement::getIndexInPanel))
            .collect(Collectors.toList()));
    controller.setSelectedFormelementsFooter(
        controller.getSelectedFormelementsFooter().stream()
            .sorted(Comparator.comparingInt(Formelement::getIndexInPanel))
            .collect(Collectors.toList()));
  }
  ...
  public void saveAttachmentsForEmail(String folderPath,
      ExpressUserEmail mail) {
  ...
  private void setPathForAttachments(String folderPath,
      List<ExpressAttachment> attachments) {
  ...
  private void removeDeletedAttachment(List<ExpressAttachment> attachments) {
    ...
    if (attachmentIter.hasNext()) {
      ...
      if (attachment.getStatus() == ExpressEmailAttachmentStatus.DELETED
  ...
  public boolean isUserTask(TaskDef task) {
    return task.getTaskType() == TaskType.USER_TASK
        || task.getTaskType() == TaskType.USER_TASK_WITH_EMAIL;
  }
   ...
   * @return check result
   */
  public boolean hasNextApprovalTask(List<TaskDef> taskList,
      int userTaskIndex) {
    List<TaskDef> subTaskList = taskList.subList(userTaskIndex + 1,
        taskList.size() - 1);
  ...
  public boolean canFinishFormDefinition(List<TaskDef> definedTasks) {
    boolean isAnyCreateFormNotDefined = definedTasks.stream()
        .filter(taskDef -> (taskDef.getTaskType() != TaskType.EMAIL
            && taskDef.getTaskType() != TaskType.APPROVAL
            && taskDef.getDragAndDropController().isNotDefined()))
        .findFirst().isPresent();

    boolean isAnyEmailEmpty = definedTasks.stream()
        .filter(taskDef -> taskDef.getTaskType() == TaskType.EMAIL
            && taskDef.getEmail().isEmpty())
        .findFirst().isPresent();
   ...
   * @return check result
   */
  public boolean displayNextButton(List<TaskDef> definedTasks,
      int currentIndex) {
    ...
    return !definedTasks.subList(currentIndex + 1, definedTasks.size()).stream()
        .filter(item -> item.getTaskType() != TaskType.APPROVAL)
        .collect(Collectors.toList()).isEmpty();
  }

  public int nextAvailableTaskIndex(List<TaskDef> definedTasks,
      int currentIndex) {
    Optional<TaskDef> taskDef = definedTasks
        .subList(currentIndex + 1, definedTasks.size()).stream()
  ...
  public List<ExternalDataProvider> findDataProviders() {
    ...
    SubProcessSearchFilter filter = subprocessFilter
        .setSignature("portalExpressDataProvider()")
        .setSearchScope(SearchScope.APPLICATION).toFilter();
    return SubProcessRunner.findSubProcessStarts(filter).stream()
        .map(this::toDataProvider).collect(Collectors.toList());
  }

  private ExternalDataProvider toDataProvider(
      ISubProcessStart subProcessStart) {
    ExternalDataProvider dataProvider = new ExternalDataProvider();
    dataProvider.setLibraryId(
        subProcessStart.getProcessModelVersion().getLibrary().getId());
