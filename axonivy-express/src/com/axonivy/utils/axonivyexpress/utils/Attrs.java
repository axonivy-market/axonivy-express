...
/**
 * <p>
 * Small utility to set/get {@code cc.attrs} in HtmlDialog logic or
 * {@code @ManagedBean}.
 * </p>
 * 
 * <p>
...
public class Attrs {
  ...
  private Attrs(FacesContext facesContext) {
    if (facesContext == null) {
      throw new IllegalArgumentException(
          "FacesContext must not be null to use Attrs");
    }
  ...
  // Set it `public` and remove the `@SuppressWarning` if you want to publish
  // it.
  ...
  private void set(String attribute, Object value) {
    ...
    ValueExpression valueExpression = facesContext.getApplication()
        .getExpressionFactory().createValueExpression(elContext,
            "#{cc.attrs." + attribute + "}", Object.class);
  ...
  private <T> T getAttribute(String attributeExpression) {
    ...
    T attributeValue = (T) fc.getApplication().evaluateExpressionGet(fc,
        attributeExpression, Object.class);
    return attributeValue;
  }

  @SuppressWarnings({ "unchecked" })
  public <T> T getAttribute(String attributeExpression,
      Class<? extends Object> expectedType) {
    FacesContext fc = FacesContext.getCurrentInstance();
    T attributeValue = (T) fc.getApplication().evaluateExpressionGet(fc,
        attributeExpression, expectedType);
    return attributeValue;
  }
}