...
import com.axonivy.utils.axonivyexpress.entity.AbstractConfiguration;
...
public abstract class JsonConfigurationService<T extends AbstractConfiguration> {
  ...
  public List<T> setPrivateConfig(List<T> entities) {
    sessionUser().setProperty(getConfigKey(),
        BusinessEntityConverter.entityToJsonValue(entities));
    return entities;
  }

  public T findById(String id) {
    return getPrivateConfig().stream().filter(e -> e.getId().equals(id))
        .findFirst().orElseGet(() -> getPublicConfig().stream()
  ...
  public List<T> getPublicConfig() {
    ...
    List<T> entities = BusinessEntityConverter.jsonValueToEntities(jsonValue,
        getType());
  ...
  public T save(T entity) {
    ...
    List<T> entities = entity.getIsPublic() ? getPublicConfig()
        : getPrivateConfig();
  ...
  protected void savePublicConfig(Object object) {
    Ivy.var().set(getConfigKey(),
        BusinessEntityConverter.entityToJsonValue(object));
  }
