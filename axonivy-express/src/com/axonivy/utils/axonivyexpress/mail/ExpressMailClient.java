...
public class ExpressMailClient {
  ...
  private static List<Attachment> getAttachments(
      List<ExpressAttachment> expressAttachments) {
    ...
    for (ExpressAttachment expressAttachment : expressAttachments) {
      ...
      try {
        ...
        Attachment mailAttachment = Attachment.create()
            .file(attachedFile.getJavaFile()).toAttachment();
  ...
  private static MailMessage mapMailMessage(ExpressUserEmail mail) {
    Builder mailMessageBuilder = MailMessage.create().subject(mail.getSubject())
  ...
  private static List<String> buildToMailAddress(String recipients) {
    ...
    if (recipients.contains(";")) {
      recipientEmails.addAll(Arrays.asList(recipients.split(";")).stream()
