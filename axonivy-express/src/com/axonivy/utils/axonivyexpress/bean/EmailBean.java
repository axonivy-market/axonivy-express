...
public class EmailBean implements Serializable {
  ...
  public void init(ExpressUserEmail email, List<String> responsibles) {
    ...
    if (this.userEmail.getRecipients() == null
        || this.userEmail.getRecipients().isEmpty()) {
  ...
  public void uploadAttachment(FileUploadEvent event) throws IOException {
    ...
    if (!StringUtils.isEmpty(uploadDocumentCheckMessage)) {
      FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(
          FacesMessage.SEVERITY_ERROR, uploadDocumentCheckMessage, null));
      return;
    }

    List<ExpressAttachment> attachments = userEmail.getAttachments();
    boolean isDuplicatedName = attachments.stream()
        .anyMatch(attachment -> (attachment
            .getStatus() != ExpressEmailAttachmentStatus.DELETED
  ...
  public void downloadAttachment(ExpressAttachment attachment)
    ...
    if (attachment.getContent() != null) {
      file = DefaultStreamedContent.builder()
          ...
          .contentType(attachment.getContentType()).name(attachment.getName())
          .build();
      ...
      file = DefaultStreamedContent.builder().stream(() -> is)
          .contentType(attachment.getContentType()).name(attachment.getName())
          .build();
    }
  }
  ...
  private void addFileDuplicationMessage(FileUploadEvent event,
    ...
    FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR,
        Ivy.cms().co(
            "/Dialogs/com/axonivy/portal/components/DocumentTable/UploadFileExists",
  ...
  private void uploadAttachmentTemporarily(FileUploadEvent event)
    ...
    attachmentDTO.setContentType(
        Files.probeContentType(Paths.get(uploadedFile.getFileName())));
  ...
  public List<ExpressAttachment> getFilteredAttachments() {
    ...
    return this.userEmail.getAttachments().stream()
        .filter(
            item -> item.getStatus() != ExpressEmailAttachmentStatus.DELETED)
        .collect(Collectors.toList());
  }

  public void setFilteredAttachments(
      List<ExpressAttachment> filteredAttachments) {
