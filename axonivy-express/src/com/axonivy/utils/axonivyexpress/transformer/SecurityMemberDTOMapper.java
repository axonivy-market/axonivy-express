package com.axonivy.utils.axonivyexpress.transformer;
...
public class SecurityMemberDTOMapper {
  ...
  public static List<SecurityMemberDTO> mapFromUserDTOs(
      List<UserDTO> userDTOs) {
    return map(userDTOs, user -> mapFromUserDTO(user));
  }

  public static List<SecurityMemberDTO> mapFromRoleDTOs(
      List<RoleDTO> roleDTOs) {
    return map(roleDTOs, role -> mapFromRoleDTO(role));
  }

  private static <T, R> List<R> map(List<T> list, Function<T, R> function) {
    return CollectionUtils.emptyIfNull(list).stream().map(function)
